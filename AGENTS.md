...
- **MUST HAVE — 17/25 done:** the base `MH-01..MH-12` are 11/11 complete (ثبت‌نام/ورود/خروج (`Register.razor`, `Login.razor`, `MainLayout.razor`) · Home + books list + details with دانلود/افزودن به کتابخانه (`Home.razor`, `BooksList.razor`, `BookDetails.razor`) · افزودن به کتابخانه + لیست کتاب‌های کاربر (`BookDetails.razor`, `MyLibrary.razor`) · افزودن/ویرایش/حذف کتاب (ادمین) (`AdminBooks.razor`, `AdminBookAdd.razor`, `AdminBookEdit.razor`) · هدایت هوشمند ناشناس — anonymous → login → auto-add / auto-download (`BookDetails.razor` pending flows)) + **`MH-16` معرفی محصول در صفحهٔ اول (✅ — unified value section on `Home.razor`: «چرا نون گرد؟» with the two pillar cards (NYT/AI) + a «همین حالا بخوانید / با کمی انتظار» strip) + `MH-23` محتوای بیشتر صفحهٔ اول (✅ — «تازه‌های کتابخانه» shelf with real covers + closing CTA band on `Home.razor`; the page flows hero → value → shelf → closing CTA, and the CTA buttons go full-width on mobile)** + `MH-24` کاهش لود مجدد پس از ورود با گوگل (✅ — SPA navigation after Google OAuth, no WASM re-boot; see the Google OAuth section's timing rule) + `MH-21` صفحهٔ بارگذاری اولیه (✅ — branded Farsi splash, see the Splash/Header/Thumbnails section) + `MH-22` نام برند کنار لوگو در هدر (✅ — «نون گرد» next to the header logo) + `MH-25` تصاویر بندانگشتی جلد کتاب (✅ — server-resized webp cover thumbnails, see the Splash/Header/Thumbnails section)** + **8 planned core features NOT yet implemented** (`MH-13`..`MH-15`, `MH-17`..`MH-20` + `MH-26`, all ❌):
  - **ترجمه (فرآیند ترجمه خارج از برنامه است):** `MH-13` بررسی درخواست‌های ترجمه (ادمین) · `MH-14` نمایش وضعیت ترجمه (ترجمه‌شده/ترجمه‌نشده) · `MH-20` درخواست ترجمه توسط کاربر · `MH-26` رأی کیفیت ترجمه توسط کاربر
...
MUST HAVE (new core pillars, not yet implemented — `MH-13`..`MH-15`, `MH-17`..`MH-20`, `MH-26`): ترجمه — `MH-13` بررسی درخواست‌های ترجمه (ادمین)، `MH-14` وضعیت ترجمه، `MH-20` درخواست ترجمه توسط کاربر، `MH-26` رأی کیفیت ترجمه توسط کاربر · نیویورک تایمز — `MH-15` کتاب‌های پرفروش نیویورک تایمز، `MH-17` مشخصات پرفروش بودن، `MH-18` سرویس جستجوی عنوان در NYT، `MH-19` بروزرسانی گروهی وضعیت NYT (ادمین)
