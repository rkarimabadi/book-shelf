...
> **Product name:** the public app is branded **«نون گرد»**. `BookStore.*` remains the internal solution/project prefix. The brand string lives in the browser tab title (`wwwroot/index.html`), the `Home` hero, and the header logo `alt` (`MainLayout.razor`) — keep those four sites in sync when renaming (there are exactly four).

## Product Vision & Scope

The two core pillars of «نون گرد» are: **(1) AI-translated Persian versions of foreign books** (fast access instead of months-long human translation) and **(2) books sourced from the New York Times bestseller list** (quality/demand proven before translation; books carry the NYT badge, weeks-on-list, and current-list status).

⚠️ **The AI translation PROCESS is OUT OF SCOPE.** Translation happens externally as an independent activity; the finished Persian file is added to the app by the admin through the regular book add/edit flows (`MH-10`/`MH-11`). The in-app translation surface is limited to: users requesting translation for not-yet-translated books and signalling priorities (`MH-20`), admin reviewing/prioritizing those requests (`MH-13`), and translated/not-translated status display with download gating (`MH-14`). There is no in-app translation pipeline/job/queue. Planned core features: `MH-13`..`MH-20` (all ❌ in `book/MVP Scope Document.md`; see the MVP Status section).
...
- **PWA:** `wwwroot/manifest.webmanifest` (name/short_name «نون گرد», `lang: fa`, `dir: rtl`, `display: standalone`, `start_url: /`, theme `#007AFF`, background `#FFFFFF`, icons `icon-192.png` 192×192 + `apple-touch-icon.png` 180×180) + `wwwroot/sw.js` (conservative app-shell worker: `/api/*` and `/uploads/books/*` network-only, `_framework/*` network-first so app code is never stale, navigation network-first with shell fallback, other same-origin static assets cache-first with background refresh; covers are safe to cache because re-uploads get fresh filenames). Registered in `index.html`: manifest link, light/dark `theme-color` metas (`#FFFFFF`/`#000000` — matches the app's `--background` tokens), iOS `apple-mobile-web-app-*` metas, `viewport-fit=cover`, and `navigator.serviceWorker.register('sw.js')`. Installable on Android (192px icon ≥ 144px requirement) and iOS Add-to-Home-Screen; note that a Blazor WASM app still needs the network to boot `_framework` (full offline is deliberately not precached).
...
- **Flow:** UI button (`ورود با گوگل` / `ثبت‌نام با گوگل`) → `GET /api/auth/google-login` (`Challenge` on the `Google` scheme, `RedirectUri` = `/api/auth/google-finalize?returnUrl=…`) → Google consent → Google redirects to `/api/auth/google-callback` (the OAuth handler's registered `CallbackPath` — the handler validates `state`, exchanges the code, signs in the `BookStore.GoogleExternal` cookie and **redirects to the `RedirectUri`, i.e. `/api/auth/google-finalize`**) → `GET /api/auth/google-finalize` reads the verified identity from the external cookie (clears it), exchanges it for the app's own JWT pair via the `ExternalLoginCommand` → redirects to `/auth/callback?access_token=…&refresh_token=…&returnUrl=…` → `GoogleCallback.razor` signs in (`AuthStateProvider.SignInWithTokensAsync` — persists the pair, invalidates the state cache and raises the auth notification exactly like the login page) and **SPA-navigates** (no `forceLoad`) to returnUrl — the WASM app is never re-booted (`MH-24`)
  - ⚠️ **Timing rule (why this works now):** the sign-in must happen only AFTER the router's **initial** navigation has settled — `GoogleCallback` waits `Task.Delay(100)` in `OnAfterRenderAsync` before calling `SignInWithTokensAsync`. Raising the notification while the initial navigation is still in flight loses the update: .NET 9's `CascadingAuthenticationState` (implicitly wrapped around every route by `AuthorizeRouteView`) defers the cascaded `Task<AuthenticationState>` via `InvokeAsync`, which races the router's navigation render — the layout's `AuthorizeView`s re-evaluate with the old anonymous cascade and stay that way (no second evaluation). Verified via Playwright/CDP with a signed dev-secret JWT: raising the notification on first render ⇒ anonymous header forever; with the settle delay ⇒ single document load (`performance.getEntriesByType('navigation').length === 1`), authenticated header on the target route, auth state persists across subsequent SPA navigation. After the initial navigation settles, the bounce behaves byte-for-byte like the email-login flow (`SignInAsync` + `NavigateTo`), which was never affected
...
- **Known limitation:** the fresh JWT pair rides the callback URL query string (standard SPA pattern; the SPA navigation immediately replaces the token URL via pushState, but the tokens remain in browser history and may appear in server access logs until the user navigates). Full E2E requires real Google credentials; verified without them: build clean, status gating, challenge redirect to accounts.google.com (correct registered `redirect_uri`), post-auth redirect targets `google-finalize`, callback error path → `/login?google_error=1`, and the signed-JWT bounce-page flow (stores tokens → SPA navigation → authenticated header, single document load — no WASM re-boot)
...
## Splash, Header Brand & Cover Thumbnails Status (MH-21, MH-22, MH-25)

- **MH-21 — branded Farsi first-load splash:** `index.html` `#app` renders a **minimal** loading state: the brand «نون گرد», the shared `LoadingSpinner` visual (`.app-loading-spinner` — same 2rem/0.25rem ring, `--fill` border + `--color-blue` top, 0.8s spin, disabled under `prefers-reduced-motion`; the splash is plain HTML because it renders before Blazor boots, so it mirrors the component's CSS instead of using it) and the short patience note «لطفاً چند لحظه صبر کنید…» (`.app-loading-note`, `--text-subhead`). Styled in `app.css` (centered flex column, design tokens). Verified: splash present during WASM boot (computed styles identical to `LoadingSpinner`'s compiled CSS), replaced by the app once booted, zero English.
- **MH-22 — brand name next to the header logo:** `MainLayout.razor` renders `<span class="app-brand-name">نون گرد</span>` beside `.app-logo` (styled in `MainLayout.razor.css`, `--text-headline`/700, visible on all breakpoints). The brand now appears in the tab title, Home hero, header logo `alt`, AND the visible header name — keep those sites in sync on rename.
- **MH-25 — server-resized cover thumbnails:** new public endpoint `GET /api/books/covers/{fileName}?w=…` (clamped 32–1024, default 256) in `BooksController` → `ICoverThumbnailGenerator.GetOrCreateThumbnailAsync` (`CoverThumbnailGenerator` in Infrastructure, **SixLabors.ImageSharp 3.1.x** — Apache-2.0; v4.x is NOT used because it requires a paid license key and breaks the 0-warnings build) resizes the cover to webp q82 at the requested width and caches it on disk at `uploads/covers/thumbs/<name>.w<w>.webp` (first request generates, later ones serve the file; `Cache-Control: public, max-age=31536000` via `[ResponseCache]` since re-uploads get fresh filenames). `IBookService.CoverThumbnailUrl` builds the URL; `BookCover` gained a `Thumbnail` parameter (default false) and the **cards** (`BookCard`), **home shelf** and **admin rows** pass `Thumbnail="true"` while the **details page keeps the full-size image**. Verified end-to-end with a generated 800×1200 PNG + a hand-inserted dev-DB book: cards/shelf load the 256px webp (180 bytes vs 56 KB original), details use `/uploads/covers/…`, cache file created, 404 for missing covers.
  - ⚠️ **Razor gotcha (pre-existing bug fixed):** component parameters without `@` render LITERALLY — the home shelf had `<BookCover Path="book.CoverImagePath" />` (no `@`), so every shelf card requested `/api/books/covers/book.CoverImagePath` and showed the placeholder; now `Path="@book.CoverImagePath"` (same class of trap as pitfall 17).
  - ⚠️ **EF Core 9 SQLite stores Guids UPPERCASE** — a hand-inserted lowercase id matches `WHERE Id = @guid` in no rows (list query worked, GetById 404'd). Insert EF-shaped test rows with uppercase ids.
...
- **Back navigation (context-aware):** `BookDetails.razor` shows a «بازگشت به فهرست کتاب‌ها» / «بازگشت به کتابخانهٔ من» link at the top of the page. The public list state (`search`/`category`/`page`) lives in the `/books` URL query string — card links carry it into the detail URL (`/books/{id}?search=…&category=…&page=…`), `BooksList` restores it via `[SupplyParameterFromQuery]` + `OnParametersSetAsync` (invalid categories dropped) and keeps the URL in sync with `SyncUrl` (replace navigation; also rewrites the URL after a page clamp). Library cards carry `?from=library` instead, pointing the link back at `/library`. «کپی لینک»/اشتراک‌گذاری strip the carried query so shared URLs stay clean.
...
## Translation Quality Rating Status (MH-26 - رأی کیفیت ترجمه توسط کاربر)

- **Requirement:** a logged-in user rates the translation quality of a book (1–5 stars); the average + vote count show on the book details page and per-book stats appear in the admin panel. Implemented **independently of `MH-14`** (no translation-status concept exists yet) — the rating applies to every book for now; a `MH-14`-style gate can be layered on later
- **Domain (Core):** `TranslationRating` aggregate (mirrors `BookNote`) — `UserId` + `BookId` + `Rating` (1–5, validated in `Create`/`Set` → `TranslationRatingErrors.InvalidRating`, 400) + `UpdatedAt`; one rating per user per book (unique index). `ITranslationRatingRepository` contract: `GetByUserAndBookAsync`, `Add`/`Update`, `GetRatingSummaryAsync(bookId)` → `(Average, Count)`, `GetAllSummariesAsync` → `Dictionary<bookId, (Average, Count)>` (only books with ≥1 rating)
- **Data:** `AddTranslationRatings` migration creates `TranslationRatings` (Rating INTEGER, unique (UserId, BookId), FK cascade on both User and Book — deleting a book/user removes its ratings); `TranslationRatingConfiguration` + `DbSet<TranslationRating>` + `TranslationRatingRepository` + DI registration
- **Application:** `Features/Ratings/` — `SaveBookRatingCommand` (upsert: update the existing row or create after `IBookRepository.ExistsAsync` → 404 `Book.NotFound`; validator `InclusiveBetween(1,5)`), `GetBookRatingQuery(userId?, bookId)` → `(Average, Count, MyRating)` (own rating null when anonymous/unrated). `GetBookQuery`/`GetBooksQuery` now also return rating summaries (via `ITranslationRatingRepository`) so the details and list/admin endpoints carry `AverageRating`/`RatingsCount`
- **Contracts:** `BookResponse` gained trailing optional `AverageRating` (double?) + `RatingsCount` (int) — all existing construction sites compile unchanged; new `MyBookRatingResponse(Average, Count, Rating)` + `SaveBookRatingRequest(Rating)`
- **API:** `BookRatingsController` (`/api/books/{bookId:guid}/rating`) — `GET` public summary (+ own rating when a valid JWT is present; anonymous → `rating: null`), `PUT` `[Authorize(Policy = Policies.RequireUserRole)]` → 204. The admin panel gets its stats through the existing `GET /api/books?includeInactive=true&pageSize=500` (no separate admin endpoint needed — `GetBooksQuery` already aggregates)
- **UI — details page (`BookDetails.razor`):** a «کیفیت ترجمه» panel (always visible): average rendered as filled stars (rounded to the nearest whole star) + «X از ۵ (N رأی)» with Persian digits, or «هنوز کسی به کیفیت ترجمهٔ این کتاب امتیاز نداده است.»; an authenticated user gets a 5-star picker (hover preview, selected state, busy spinner while saving) that upserts their vote and refreshes the summary; **anonymous users see a «برای امتیاز دادن وارد شوید» button → `/login?returnUrl=/books/{id}`** (same smart-login pattern as add-to-library; after login the user is back on the book and can rate). Errors map to Persian toasts (`TranslationRating.InvalidRating` → «امتیاز باید بین ۱ تا ۵ باشد.»)
- **UI — admin panel (`AdminBookRow`):** an orange «میانگین امتیاز کیفیت ترجمه» pill per book — «۳٫۵ از ۵ (۲ رأی)» or «بدون امتیاز» — populated from the `BookResponse` the admin list already loads (no extra request)
- `PersianDateFormatter.ToPersianDigits` was made public (was private) so the rating labels reuse the standard Persian-digit conversion
- No schema risk (unique index + cascades verified in the migration). Verified via API smoke test: anon GET → `{average: null, count: 0, rating: null}` · PUT as a fresh user → 204 · GET → `{average: 5, count: 1, rating: 5}` · overwrite 5→3 keeps `count: 1` (one row per user) · rating 9 → 400 · anon PUT → 401 · two users (3 + 4) → `average: 3.5, count: 2` in both the details and list endpoints · admin `includeInactive=true&pageSize=500` carries the stats. Full solution builds 0 warnings / 0 errors
...
- **MUST HAVE — 18/25 done:** the base `MH-01..MH-12` are 11/11 complete (ثبت‌نام/ورود/خروج (`Register.razor`, `Login.razor`, `MainLayout.razor`) · Home + books list + details with دانلود/افزودن به کتابخانه (`Home.razor`, `BooksList.razor`, `BookDetails.razor`) · افزودن به کتابخانه + لیست کتاب‌های کاربر (`BookDetails.razor`, `MyLibrary.razor`) · افزودن/ویرایش/حذف کتاب (ادمین) (`AdminBooks.razor`, `AdminBookAdd.razor`, `AdminBookEdit.razor`) · هدایت هوشمند ناشناس — anonymous → login → auto-add / auto-download (`BookDetails.razor` pending flows)) + **`MH-16` معرفی محصول در صفحهٔ اول (✅ — unified value section on `Home.razor`: «چرا نون گرد؟» with the two pillar cards (NYT/AI) + a «همین حالا بخوانید / با کمی انتظار» strip) + `MH-23` محتوای بیشتر صفحهٔ اول (✅ — «تازه‌های کتابخانه» shelf with real covers + closing CTA band on `Home.razor`; the page flows hero → value → shelf → closing CTA, and the CTA buttons go full-width on mobile)** + `MH-24` کاهش لود مجدد پس از ورود با گوگل (✅ — SPA navigation after Google OAuth, no WASM re-boot; see the Google OAuth section's timing rule) + `MH-21` صفحهٔ بارگذاری اولیه (✅ — branded Farsi splash, see the Splash/Header/Thumbnails section) + `MH-22` نام برند کنار لوگو در هدر (✅ — «نون گرد» next to the header logo) + `MH-25` تصاویر بندانگشتی جلد کتاب (✅ — server-resized webp cover thumbnails, see the Splash/Header/Thumbnails section) + `MH-26` رأی کیفیت ترجمه توسط کاربر (✅ — 1–5 star ratings, summary + star picker on the details page, per-book stats in the admin panel; see the Translation Quality Rating Status section)** + **7 planned core features NOT yet implemented** (`MH-13`..`MH-15`, `MH-17`..`MH-20`, all ❌):
  - **ترجمه (فرآیند ترجمه خارج از برنامه است):** `MH-13` بررسی درخواست‌های ترجمه (ادمین) · `MH-14` نمایش وضعیت ترجمه (ترجمه‌شده/ترجمه‌نشده) · `MH-20` درخواست ترجمه توسط کاربر
  - **نیویورک تایمز:** `MH-15` کتاب‌های پرفروش نیویورک تایمز (منبع کتاب‌ها + نشان) · `MH-17` مشخصات پرفروش بودن (مدت حضور + حضور فعلی در لیست) · `MH-18` سرویس جستجوی عنوان در لیست NYT · `MH-19` بررسی/بروزرسانی گروهی وضعیت NYT (ادمین)
- **SHOULD HAVE — 10/11 done:** `SH-01` حذف از کتابخانه · `SH-02` جستجو · `SH-03` صفحه‌بندی · `SH-04` بازیابی رمز عبور · `SH-05` فعال/غیرفعال کردن کتاب · `SH-06` پیام‌های عملیاتی (Toast) · `SH-07` مدیریت کاربران · `SH-08` تغییر رمز عبور · `SH-09` ناوبری واکنش‌گرا · `SH-10` تغییر/بازنشانی رمز عبور توسط ادمین — remaining: `SH-11` نشان‌گذاری وضعیت کتاب در کتابخانه (خوانده‌شده / در فهرست علاقه‌مندی‌ها)
...
- **EXTRA (no feature code):** ورود/ثبت‌نام با گوگل (Google OAuth, see its status section) · رابط تمام‌فارسی (Farsi Localization Status) · تاریخ شمسی (Persian Solar Calendar Dates Status) · PWA (manifest + service worker, see the Technology Stack bullet) · بازگشت هوشمند از صفحهٔ مشخصات کتاب به فهرست/کتابخانه با حفظ فیلترها (see Public Books UI Status)

## Next Steps (remaining features)

MUST HAVE (new core pillars, not yet implemented — `MH-13`..`MH-15`, `MH-17`..`MH-20`): ترجمه — `MH-13` بررسی درخواست‌های ترجمه (ادمین)، `MH-14` وضعیت ترجمه، `MH-20` درخواست ترجمه توسط کاربر · نیویورک تایمز — `MH-15` کتاب‌های پرفروش نیویورک تایمز، `MH-17` مشخصات پرفروش بودن، `MH-18` سرویس جستجوی عنوان در NYT، `MH-19` بروزرسانی گروهی وضعیت NYT (ادمین)
SHOULD HAVE: `SH-11` نشان‌گذاری وضعیت کتاب در کتابخانه (خوانده‌شده / در فهرست علاقه‌مندی‌ها)
